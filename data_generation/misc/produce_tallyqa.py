...
def process_data(num, json_data, client,output_file):
    ...
    boundingbox = data['bboxs']
    dataset_name = data['dataset']
    response_gt = data['answer']
    image_path = 'data/GRIT_data/' + data['image']
    ...
    SYSTEM_PROMPT = f'''Please help me construct data that meets the specified think-answer format with English. 
    ...
    <area>[{{"bbox_2d":[0.238,0.246,0.254,0.269],"depth":0.274}},{{"bbox_2d":[0.266,0.274,0.301,0.310],"depth":0.276}}]</area>
    <text>Upon closer inspection, I focus on these two areas: the skirt and the decorations, and confirm that she is wearing a pink dress.</text>
    </think>
    <answer>
    ...
    image = cv2.imread(image_path)
    height, width, _ = image.shape
    org_boundingboxlist = boundingbox
    boundingboxlist = []
    for point in org_boundingboxlist:
        normalized_point = []
        for i, coord in enumerate(point):
            if i in [0, 2]:  # x 坐标
                normalized_coord = coord / width
            else:  # y 坐标
                normalized_coord = coord / height
            # 保留三位小数并添加到结果中
            normalized_point.append(round(normalized_coord, 3))
        boundingboxlist.append(normalized_point)
    # 开始膨胀和合并边界框
...
def main(json_file, client):
    ...
    output_file = "tallyqa_data_400.json"

    # 创建线程池
    with ThreadPoolExecutor(max_workers=30) as executor:
        for num in range(min(500, len(json_data))):
            executor.submit(process_data, num, json_data, client, output_file)

    print(f"数据已成功保存到{output_file}文件中。")

if __name__ == "__main__":
    json_file = "data/GRIT_data/filtered_tallyqa_sampled.json"  # 请替换为你的JSON文件路径
    client = OpenAI(
