...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        imgs_record = [x["img_path_str"] for x in inputs]
        ...
        progress_response_value = None
        for i, reward_func in enumerate(self.reward_funcs):
            if isinstance(reward_func, PreTrainedModel):
                reward_func_name = reward_func.config._name_or_path.split("/")[-1]
            else:
                reward_func_name = reward_func.__name__
            if reward_func_name == 'response_progress_reward':
                progress_response_value = reward_per_func[i]
            self._metrics[f"rewards/{reward_func_name}"].append(reward_per_func[i].item())
        self._metrics["reward"].append(self.accelerator.gather_for_metrics(rewards).mean().item())

        if progress_response_value:
            prompt_cache = prompts[0][0]['content']
            quest = prompt_cache[-1]['text'][1:].replace(' Please first output the thinking process in <think> </think> tags, where bounding box is enclosed in <area> </area> tags and text analysis is enclosed in <text> </text> tags, alternating between them to iteratively refine the focused area. Then output the final answer in <answer> </answer> tags.','')
            has_record = False
            for item in reward_record:
                if item['prompt'] == quest and item['img'] == imgs_record[0]:
                    item['score'] = progress_response_value.item()
                    has_record = True
                    break
            if not has_record:
                reward_record.append({
                    "prompt": quest,
                    "img": imgs_record[0],
                    "score": progress_response_value.item()
                })
