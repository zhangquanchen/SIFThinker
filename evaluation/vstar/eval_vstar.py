...
def eval_model(args):
	...
	for test_type in ['direct_attributes', 'relative_position']:
		results[test_type] = []
		with open("vstar_bench/test_questions.jsonl", 'r', encoding='utf-8') as file:
			for line in file:
				result_single_sample = {}
				# 将每行解析为 JSON 对象
				data = json.loads(line)
				if data["category"] != test_type:
					continue
				# 打印每条结果
				image_pth = os.path.join(args.benchmark_folder, data['image'])
				prompt = data['text']
				label = data['label']
				...
				result = client.chat.completions.create(messages=messages, model="test",max_tokens=2048)
				response_text = result.choices[0].message.content
				response_choice = extract_answer(response_text)
				option_chosen = 1
				try:
					if label in response_choice:
						option_chosen = 0
					# print(f"response:{response_choice}, label:{label}, prompt: {prompt}, image_pth: {image_pth}")
				except:
					response_choice = ""
				...
				result_single_sample['question'] = prompt
				result_single_sample['options'] = label
				# result_single_sample['image'] = image_file
				result_single_sample['prediction_freeform'] = response_choice
