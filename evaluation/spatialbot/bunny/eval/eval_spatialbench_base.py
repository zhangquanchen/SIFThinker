...
def main():
    ...
    for idx in range(len(data_json)):
        ...
        if type(data['image']) is str: 
            data['image'] = ['SpatialBot/eval/spatial_bench/spatial_bench/' + data['image']]

        base_dir = os.path.dirname(data['image'][0])
        file_name = os.path.splitext(os.path.basename(data['image'][0]))[0]
        output_path = os.path.join(base_dir, f"{file_name}_d.png")
        data['image'].append(output_path)
        ...
        if str(data['id']).split('_')[0] == 'grounding':
            ...
            dataset.append({'id': data['id'], 'question': data['question'], 'answer': data['answer'],
                    'image': data['image']})
    ...
    for sample in dataset:
        ...
        # with torch.no_grad():
        base64_image = encode_image(sample['image'][0])
        base64_depth_image = encode_image(sample['image'][1])
        # messages = [
        #     {
        #         "role": "user",
        #         "content": [
        ...
        #             {"type": "text", "text": "\n" + sample['question'] + ' Please first output the thinking process in <think> </think> tags, where bounding box is enclosed in <area> </area> tags and text analysis is enclosed in <text> </text> tags, alternating between them to iteratively refine the focused area. Then output the final answer in <answer> </answer> tags.'},
        #         ],
        #     }]
        print(sample['question'])
        try:
            response = chat_o3(sample['question'], sample['image'][0])
        except:
            response = ""
        print(response)
        ...
        # response = extract_answer(response_text)
        if response:
            responses.append({'response':response,'gt':sample['answer'],'image':sample['image'],'question': sample['question']})
        else:
            responses.append({'response':sample['answer'],'gt':sample['answer'],'image':sample['image'],'question': sample['question']})
        print({'response':response,'gt':sample['answer']})
        j = j+1

    question_type = args.question.strip()
    with open(f'output/result-{model_name}-{question_type}.json', 'w', encoding='utf-8') as f:
